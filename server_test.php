...
<?php
    ...
    if  (isset($_POST['register'])) {
        ...
        if  (count($errors) == 0)   {
            ...
            $tasksdb=str_replace(".","",$email);
            $tasksdb=str_replace("@","",$tasksdb);
            $sql = "USE tasks; CREATE TABLE '$tasksdb' (name varchar(255), people varchar(255), date date)";
            //SET @tasksdb="cccom"; USE tasks; CREATE TABLE tasksdb (name varchar(255), people varchar(255), date date)
            /*
            SET @tasksdb = "cccom";
SET @SQL = CONCAT('CREATE TABLE ',CONCAT('tasks.',@tasksdb),' LIKE ',CONCAT(database_name,'.',table_to_create));
...
*/
            ...
            $_SESSION['tasksdb'] = $tasksdb;
            $password_1 = md5($password_1); //encrypts password before storing in database
            $sql = "INSERT INTO users (name, email, password, tasksdb, profile_picture) VALUES ('$name', '$email', '$password_1', '$tasksdb', '$imgContent')";
            mysqli_query($db, $sql);
